  ...
  const handleDelete = async (id) => {
    try {
      ...
      const updatedTotalPages = Math.max(
        1,
        Math.ceil((articles.length - 1) / ARTICLES_PER_PAGE),
      );
      ...
      <header className="admin-page-header">
        <div>
          ...
          <p className="admin-page-description">
            Review, edit, and manage your published content.
          </p>
        </div>
        <div className="admin-article-count">
                ...
                <article className="card admin-article-card h-100">
                  ...
                  <div className="card-body d-flex flex-column">
                    <div className="admin-article-card-topline">
                      <span
                        className={`admin-status admin-status-${item.status || "published"}`}
                      >
                        {item.status || "published"}
                      </span>
                      <span className="admin-article-category">
                    ...
                    <p className="card-text">
                      {item.content
                        ? `${htmlToText(item.content).substring(0, 120)}...`
                        : "No content"}{" "}
                    </p>
